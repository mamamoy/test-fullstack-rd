...
const product_code = ref(props.code)
const minimum_qty = ref('')
// end
...
const filteredTable = computed(() => {
    if (!searchTable.value) {
        ...
        return props.products.filter(data => data.product_name.toLowerCase().includes(query) || data.product_code.toLowerCase().includes(query) || data.minimum_qty.toString().includes(query));
    }
})
// end
...
const openEditModal = (product) => {
    ...
    minimum_qty.value = product.minimum_qty
    ...
    <el-dialog v-model="dialogVisible" :title="editMode ? 'Edit product' : 'Add new product'" width="500"
        ...
        <form @submit.prevent="editMode ? updateProduct() : addProduct()" class="max-w-md mx-auto">
            <div class="relative z-0 w-full mb-5 group">
                <input type="text" v-model="product_name" name="product_name" id="product_name"
            ...
            <div class="relative z-0 w-full mb-5 group">
                <input type="text" v-model="product_code" name="product_code" id="product_code"
                    class="block py-2.5 px-0 w-full text-sm text-gray-900 bg-transparent border-0 border-b-2 border-gray-300 cursor-not-allowed appearance-none dark:text-white dark:border-gray-600 dark:focus:border-blue-500 focus:outline-none focus:ring-0 focus:border-blue-600 peer"
            ...
            <div class="relative z-0 w-full mb-5 group">
                <input type="number" v-model="minimum_qty" name="minimum_qty" id="minimum_qty"
                    class="block py-2.5 px-0 w-full text-sm text-gray-900 bg-transparent border-0 border-b-2 border-gray-300 appearance-none dark:text-white dark:border-gray-600 dark:focus:border-blue-500 focus:outline-none focus:ring-0 focus:border-blue-600 peer"
                    placeholder=" " required />
                <label for="minimum_qty"
                    class="peer-focus:font-medium absolute text-sm text-gray-500 dark:text-gray-400 duration-300 transform -translate-y-6 scale-75 top-3 -z-10 origin-[0] peer-focus:start-0 rtl:peer-focus:translate-x-1/4 rtl:peer-focus:left-auto peer-focus:text-blue-600 peer-focus:dark:text-blue-500 peer-placeholder-shown:scale-100 peer-placeholder-shown:translate-y-0 peer-focus:scale-75 peer-focus:-translate-y-6">Minimum
                    Quantity</label>
            </div>
            <button type="submit"
